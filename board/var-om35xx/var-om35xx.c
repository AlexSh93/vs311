...
#define MUX_DEFAULT()\
	...
	MUX_VAL(CP(ETK_D14),        (IDIS | PTU | DIS | M4)) /*Backlight*/\
...
/**********************************************************
 * Routine: set_LCD_gpios
 * Description: Setting up the configuration GPIO registers
 *              specific to the LCD hardware. The GPIO  
 *              clock are enabled as well.
 *********************************************************/
void set_LCD_gpios(void)
{
	u32 val;
	...
	/* Enable I and F Clocks for GPIO1 */
	val = __raw_readl(CM_ICLKEN_WKUP) | CM_CLKEN_GPIO1;
	__raw_writel(val, CM_ICLKEN_WKUP);
	val = __raw_readl(CM_FCLKEN_WKUP) | CM_CLKEN_GPIO1;
	__raw_writel(val, CM_FCLKEN_WKUP);

	/* Enable I and F Clocks for GPIO2-GPIO6 */
	val = __raw_readl(CM_ICLKEN_PER) | CM_CLKEN_GPIO2 | CM_CLKEN_GPIO3 | CM_CLKEN_GPIO4 | CM_CLKEN_GPIO5 | CM_CLKEN_GPIO6;
	__raw_writel(val, CM_ICLKEN_PER);
	val = __raw_readl(CM_FCLKEN_PER) | CM_CLKEN_GPIO2 | CM_CLKEN_GPIO3 | CM_CLKEN_GPIO4 | CM_CLKEN_GPIO5 | CM_CLKEN_GPIO6;
	__raw_writel(val, CM_FCLKEN_PER);
	
	__raw_writel(GPIO3 , gpio5_base + OFFS(GPIO_CLEARDATAOUT));
	__raw_writel(__raw_readl(gpio1_base + OFFS(GPIO_OE)) & (~(GPIO28)), gpio1_base + OFFS(GPIO_OE));

	__raw_writel(GPIO28, gpio1_base + OFFS(GPIO_SETDATAOUT));
	__raw_writel(__raw_readl(gpio5_base + OFFS(GPIO_OE)) & (~(GPIO3)) , gpio5_base + OFFS(GPIO_OE));
}
